...
var (
	...
	ErrEmptyStringEnvVar    = errors.New("empty value for environment variable")
...
func (r *EnvReader) Key() (string, error) {
	if r.CredKey == "" {
		return "", fmt.Errorf("%w: %s", ErrEmptyStringEnvVar, r.EnvName)
	}
...
func (r *JSONReader) Key() (string, error) {
	if r.CredKey == "" {
		return "", fmt.Errorf("%w: %s", ErrEmptyFilePathPathVar, r.FilePath)
	}
