	...
	tests := []testroutines.Delete{
		...
		},
		{
			...
			ExpectedErrs: []error{
				...
				testutils.StringError("invalid payload; company not found"),
			},
		},
		{
			Name:  "Remove Company",
		...
		{
			Name:  "Remove Notebooks",
			Input: common.DeleteParams{ObjectName: "notebooks", RecordId: "1433336"},
			Server: mockserver.Conditional{
				Setup: mockserver.ContentJSON(),
				If: mockcond.And{
					mockcond.MethodDELETE(),
					mockcond.Path("/projects/api/v3/notebooks/1433336.json"),
				},
				Then: mockserver.Response(http.StatusNoContent),
			}.Server(),
			Expected:     &common.DeleteResult{Success: true},
