...
func extractMetadataRecords(res map[string]any, objectName string) ([]any, error) {
	if objectName == "callanalysis" {
		...
		if !ok {
			return nil, fmt.Errorf("couldn't convert the data response field data to a map: %w", common.ErrMissingExpectedValues)
		}

		records, ok := data["data"].([]any)
		if !ok {
			return nil, fmt.Errorf("couldn't convert the nested data response field data to an array: %w", common.ErrMissingExpectedValues) // nolint:lll
		}
