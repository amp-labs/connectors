...
func (c *Connector) buildWriteURL(params common.WriteParams) (*urlbuilder.URL, string, error) {
	...
	case objectEvents:
		if params.IsCreate() {
			u, err := buildURLWithVersion(base, "", "events")

			return u, http.MethodPost, err
		}

		u, err := buildURLWithVersion(base, "", "events", params.RecordId)

		return u, http.MethodPatch, err
	case objectUsers:
		u, err := buildURLWithVersion(base, "", "users")
	...
	case objectSessionPeopleBulk:
		...
		u, err := buildURLWithVersion(base, "", "sessions", sessionID, "people", "bulk")
