...
func (c *Connector) Write(ctx context.Context, config common.WriteParams) (*common.WriteResult, error) {
	...
	if len(config.RecordId) == 0 {
		...
		if supportedObjectsByUpdatePUT[c.Module()].Has(config.ObjectName) {
			write = c.JSONHTTPClient().Put

			url.AddPath(config.RecordId)
		}

		if supportedObjectsByUpdatePATCH[c.Module()].Has(config.ObjectName) {
	...
	return constructWriteResult(config, c.Module(), body)
}
