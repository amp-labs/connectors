...
func TestRead(t *testing.T) { //nolint:funlen,maintidx
	...
	responseListMembersEmpty := testutils.DataFromFile(t, "read/list-members-empty.json")
	...
	responseAlertsSettings := testutils.DataFromFile(t, "read/alerts-settings.json")
	responseKeys := testutils.DataFromFile(t, "read/keys.json")
	responseThresholdHits := testutils.DataFromFile(t, "read/thresholds-hits.json")
	...
	tests := []testconn.TestCaseRead{
		...
		{
			// alerts/settings records sit under "events" beside the channel config
			// objects in the same envelope; responseKey picks the array. Envelope
			// shape from the live response, item shape from the spec.
			Name: "Alerts settings reads the events array from the settings envelope",
			Input: common.ReadParams{
				ObjectName: "alerts/settings",
				Fields:     connectors.Fields("event_type", "channel"),
			},
			Server: mockserver.Switch{
				Setup: mockserver.ContentJSON(),
				Cases: []mockserver.Case{{
					If:   mockcond.And{mockcond.MethodGET(), mockcond.Path("/v1/alerts/settings")},
					Then: mockserver.Response(http.StatusOK, responseAlertsSettings),
				}},
				Default: mockserver.ResponseString(http.StatusInternalServerError, `{"message":"unexpected"}`),
			}.Server(),
			Comparator: testconn.ComparatorSubsetRead,
			Expected: &common.ReadResult{
				Rows: 2,
				Data: []common.ReadResultRow{
					{Id: "a1", Fields: map[string]any{"event_type": "ip_listed"}, Raw: map[string]any{"channel": "email"}},
					{Id: "a2", Fields: map[string]any{"event_type": "domain_reputation_decreased"}, Raw: map[string]any{"channel": "webhook"}},
				},
				NextPage: "",
				Done:     true,
			},
		},
		{
			...
			Server: mockserver.Switch{
				Setup: mockserver.ContentJSON(),
				Cases: []mockserver.Case{{
					If:   mockcond.And{mockcond.MethodGET(), mockcond.Path("/v1/thresholds/hits")},
					Then: mockserver.Response(http.StatusOK, responseThresholdHits),
				}},
				Default: mockserver.ResponseString(http.StatusInternalServerError, `{"message":"unexpected"}`),
			}.Server(),
			Comparator: testconn.ComparatorSubsetRead,
		...
		{
			// dynamic_pools/history pages by paging.next like the other cursor
			// objects (the live API returns lowercase paging keys).
			Name: "Dynamic pool history follows the paging cursor",
			Input: common.ReadParams{
				ObjectName: "dynamic_pools/history",
				Fields:     connectors.Fields("domain_name", "new_band"),
			},
			Server: mockserver.Switch{
				Setup: mockserver.ContentJSON(),
				Cases: []mockserver.Case{{
					If:   mockcond.And{mockcond.MethodGET(), mockcond.Path("/v1/dynamic_pools/history")},
			...
			Expected: &common.ReadResult{
				Rows: 1,
				Data: []common.ReadResultRow{
					{Id: "e1", Fields: map[string]any{"domain_name": "mail.example.com", "new_band": "dynamic_poor"}, Raw: map[string]any{"id": "e1"}},
				},
				NextPage: "https://api.mailgun.net/v1/dynamic_pools/history?page=next1",
				Done:     false,
			},
		},
		{
		...
		},
		{
			...
			Expected: &common.ReadResult{
				Rows: 2,
				Data: []common.ReadResultRow{
					{Id: "1", Fields: map[string]any{"id": "1"}, Raw: map[string]any{"id": "1"}},
					{Id: "2", Fields: map[string]any{"id": "2"}, Raw: map[string]any{"id": "2"}},
				},
		...
		{
			// keys timestamps are zone-less ISO 8601 UTC ("2024-06-21T03:17:35",
			// live-captured). They must parse for Since to filter; a record with an
			// empty timestamp (the account public key) is kept, never dropped.
			Name: "Since filters keys by zone-less ISO timestamps and keeps unparsable ones",
			Input: common.ReadParams{
				ObjectName: "keys",
				Fields:     connectors.Fields("id", "kind"),
				Since:      time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
			},
			Server: mockserver.Switch{
				Setup: mockserver.ContentJSON(),
				Cases: []mockserver.Case{{
					If:   mockcond.And{mockcond.MethodGET(), mockcond.Path("/v1/keys")},
					Then: mockserver.Response(http.StatusOK, responseKeys),
				}},
				Default: mockserver.ResponseString(http.StatusInternalServerError, `{"message":"unexpected"}`),
			}.Server(),
			Comparator: testconn.ComparatorSubsetRead,
			Expected: &common.ReadResult{
				Rows: 2,
				Data: []common.ReadResultRow{
					{Id: "44ec4598-a2009175", Fields: map[string]any{"kind": "user"}, Raw: map[string]any{"id": "44ec4598-a2009175"}},
					{Id: "pubkey-sample-account-public-key", Fields: map[string]any{"kind": "public"}, Raw: map[string]any{"updated_at": ""}},
				},
				NextPage: "",
				Done:     true,
			},
		...
		},
		{
			...
			Expected: &common.ReadResult{
				Rows: 2,
				Data: []common.ReadResultRow{
					{Id: "newsletter", Fields: map[string]any{"tag": "newsletter"}, Raw: map[string]any{"tag": "newsletter"}},
					{Id: "onboarding", Fields: map[string]any{"tag": "onboarding"}, Raw: map[string]any{"tag": "onboarding"}},
				},
				NextPage: "2",
				Done:     false,
			},
		...
		},
		{
			...
			Expected: &common.ReadResult{
				Rows: 2,
				Data: []common.ReadResultRow{
					{Id: "alice@example.com", Fields: map[string]any{"address": "alice@example.com"}, Raw: map[string]any{"address": "alice@example.com"}},
					{Id: "bob@example.com", Fields: map[string]any{"address": "bob@example.com"}, Raw: map[string]any{"address": "bob@example.com"}},
				},
				NextPage: "",
				Done:     true,
			},
		...
		{
			// Lists exist but hold zero members: Data must be an empty array,
			// not null (deep comparison distinguishes nil from empty).
			Name: "Nested lists/members with zero members returns empty array",
			Input: common.ReadParams{
				ObjectName: "lists/members",
				Fields:     connectors.Fields("address"),
			},
			Server: mockserver.Switch{
				...
				Cases: []mockserver.Case{
					...
					{
						If:   mockcond.And{mockcond.MethodGET(), mockcond.Path("/v3/lists/developers/members")},
						Then: mockserver.Response(http.StatusOK, responseListMembersEmpty),
					},
				},
				Default: mockserver.ResponseString(http.StatusInternalServerError, `{"message":"unexpected"}`),
			}.Server(),
