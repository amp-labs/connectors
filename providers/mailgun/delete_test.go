...
func TestDelete(t *testing.T) { //nolint:funlen
	...
	tests := []testconn.TestCaseDelete{
		...
		{
			// alerts/settings deletes at the /events sub-path, not the read path.
			Name: "Delete alert uses the settings events item path",
			Input: common.DeleteParams{
			...
			Server: mockserver.Conditional{
				...
				Then: mockserver.Response(http.StatusOK, []byte(`{"message":"Alert settings deleted"}`)),
			}.Server(),
			Expected: &common.DeleteResult{Success: true},
		},
		{
