...
func TestWrite(t *testing.T) { //nolint:funlen,maintidx
	...
	tests := []testconn.TestCaseWrite{
		...
		{
			// A record in the shape Read returns ("actions": [...]) must be
			// accepted: the list is sent as the repeated "action" form field the
			// spec defines, and the create envelope {message, route} is unwrapped.
			Name: "Create route maps read-shaped actions to repeated action fields",
			Input: common.WriteParams{
				ObjectName: "routes",
				RecordData: map[string]any{
					"expression":  "match_recipient('.*@example.com')",
					"actions":     []any{`forward("https://example.com/inbound")`, "stop()"},
		...
		{
			// A record in the shape Read returns (nested "forward" object) must be
			// accepted: it is flattened to the dotted forward.url / forward.recipient
			// query parameters, repeated per element as the spec allows.
			Name: "Create forward expands the read-shaped forward object into repeated query parameters",
			Input: common.WriteParams{
				ObjectName: "forwards",
				RecordData: map[string]any{
					"match": "inbound@example.com",
					"forward": map[string]any{
						"urls":       []any{"https://example.com/a", "https://example.com/b"},
			...
			Server: mockserver.Conditional{
				...
				If: mockcond.And{
					...
					mockcond.QueryParam("forward.url", "https://example.com/a", "https://example.com/b"),
					mockcond.QueryParam("forward.recipient", "ops@example.com"),
					mockcond.QueryParamsMissing("forward"),
				},
		...
		},
		{
			...
			Name: "Create template substitutes domain and reads the template envelope",
			Input: common.WriteParams{
				ObjectName: "templates",
				RecordData: map[string]any{
					"name":        "welcome",
					"description": "Welcome email",
					"template":    "<html>Hi {{name}}</html>",
				},
			},
			Server: mockserver.Conditional{
				Setup: mockserver.ContentJSON(),
		...
		{
			// alerts/settings is the one JSON-bodied write; it is read from the
			// settings envelope but created at the /events sub-path, and the create
			// response is the flat record (shape confirmed live).
			Name: "Create alert posts a JSON body to the settings events path",
			Input: common.WriteParams{
				ObjectName: "alerts/settings",
				RecordData: map[string]any{
					"event_type": "ip_listed",
					"channel":    "email",
					"settings":   map[string]any{"emails": []any{"ops@example.com"}},
				},
			},
			Server: mockserver.Conditional{
				Setup: mockserver.ContentJSON(),
				If: mockcond.And{
					mockcond.MethodPOST(),
					mockcond.Path("/v1/alerts/settings/events"),
					mockcond.Header(http.Header{"Content-Type": []string{"application/json"}}),
					mockcond.Body(`{"channel":"email","event_type":"ip_listed","settings":{"emails":["ops@example.com"]}}`),
				},
				Then: mockserver.Response(http.StatusOK, responseAlertCreate),
			}.Server(),
			Comparator: testconn.ComparatorSubsetWrite,
			Expected: &common.WriteResult{
				Success:  true,
		...
		{
			// Alert updates PUT the /events item path with the full JSON record;
			// the response is only {message}, so RecordId falls back to the input.
			Name: "Update alert puts JSON to the settings events item path",
			Input: common.WriteParams{
				ObjectName: "alerts/settings",
				RecordId:   "a1",
				RecordData: map[string]any{
			...
			Server: mockserver.Conditional{
				...
				If: mockcond.And{
					mockcond.MethodPUT(),
					mockcond.Path("/v1/alerts/settings/events/a1"),
					mockcond.Body(`{"channel":"email","event_type":"ip_delisted","settings":{"emails":["ops@example.com"]}}`),
				},
				Then: mockserver.Response(http.StatusOK, []byte(`{"message":"Alert settings updated"}`)),
			}.Server(),
			Comparator: testconn.ComparatorSubsetWrite,
			Expected: &common.WriteResult{
				Success:  true,
				RecordId: "a1",
				Data:     map[string]any{"message": "Alert settings updated"},
			},
		},
		{
	...
	workspaceByTest := map[string]string{
		...
		"Create template substitutes domain and reads the template envelope": "example.com",
	}
