...
func buildWriteBody(params common.WriteParams) ([]byte, string, error) {
	...
	switch params.ObjectName {
	case objectDialsession, objectFolders:
		// JSON payload.
	...
	case objectContacts, objectMembers:
...
func parseWriteResponse(
	...
	switch params.ObjectName {
	case objectContacts:
		return parseContactsWriteResponse(body, params.RecordId)
	case objectMembers:
		return parseMembersWriteResponse(body, params.RecordId)
	case objectFolders:
		return parseFoldersWriteResponse(body, params.RecordId)
	case objectDialsession:
		return parseDialsessionWriteResponse(body)
	default:
		return nil, common.ErrOperationNotSupportedForObject
	}
}

func parseContactsWriteResponse(body *ajson.Node, fallbackID string) (*common.WriteResult, error) {
	contactNode, err := jsonquery.New(body, objectContacts).ObjectOptional(objectContacts)
	if err != nil {
		return nil, err
	}

	if contactNode == nil {
		return &common.WriteResult{Success: true, RecordId: fallbackID}, nil
	}

	recordID, err := jsonquery.New(contactNode).TextWithDefault("contact_user_id", fallbackID)
	if err != nil {
		return nil, err
	}
...
func parseMembersWriteResponse(body *ajson.Node, fallbackID string) (*common.WriteResult, error) {
	memberNode, err := jsonquery.New(body, objectMembers).ObjectOptional(objectMembers)
	if err != nil {
		return nil, err
	}
	...
	recordID, err := jsonquery.New(memberNode).TextWithDefault("user_id", fallbackID)
	if err != nil {
		return nil, err
	}
...
func parseFoldersWriteResponse(body *ajson.Node, fallbackID string) (*common.WriteResult, error) {
	foldersNode, err := jsonquery.New(body).ObjectOptional(objectFolders)
	if err != nil {
		return nil, err
	}
...
func parseDialsessionWriteResponse(body *ajson.Node) (*common.WriteResult, error) {
