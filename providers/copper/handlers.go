...
func (c *Connector) buildReadRequest(ctx context.Context, params common.ReadParams) (*http.Request, error) {
	...
	method, jsonData, err := createWriteOperation(url, params)
	if err != nil {
		return nil, err
	}
...
func createWriteOperation(url *urlbuilder.URL, params common.ReadParams) (string, []byte, error) {
	...
	if strings.HasSuffix(url.Path(), "/users/search") {
		...
		if !params.Since.IsZero() {
			payload["minimum_modified_date"] = strconv.FormatInt(params.Since.Unix(), 10)
		}

		if !params.Until.IsZero() {
			payload["maximum_modified_date"] = strconv.FormatInt(params.Until.Unix(), 10)
		}
	}
	...
	return http.MethodPost, jsonData, nil
}
