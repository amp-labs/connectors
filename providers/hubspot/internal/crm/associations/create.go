...
func (s Strategy) BatchCreate( // nolint:cyclop
	ctx context.Context, objectName common.ObjectName, params *BatchCreateParams,
	...
	objectSchema, warning, err := s.fetchObjectSchema(ctx, objectName)
	if err != nil {
		return nil, err
	}
	...
	registry, warning := params.makePayloadsForRelationships(objectName, objectSchema)
	if warning != nil {
		result.AddError(warning)
	}
