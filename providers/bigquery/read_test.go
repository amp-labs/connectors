...
func TestValidateServiceAccountCredentials(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			creds:   `{"type": "external_account", "audience": "https://evil.com"}`,
			wantErr: true,
		},
		{
			name:    "authorized user rejected",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validateServiceAccountCredentials([]byte(tt.creds))
			if (err != nil) != tt.wantErr {
				t.Errorf("validateServiceAccountCredentials() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
