...
func messagesEmbedMessageFields(messages MessageRecords) readhelper.SelectedFieldsFunc {
...
func draftsEmbedMessageFields(messages MessageRecords) readhelper.SelectedFieldsFunc {
	return func(node *ajson.Node, fields []string) (map[string]any, string, error) {
		root, err := jsonquery.Convertor.ObjectToMap(node)
		if err != nil {
			return nil, "", err
		...
		filtered := readhelper.SelectFields(root, datautils.NewSetFromList(fields))
