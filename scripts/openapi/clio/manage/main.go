...
// Endpoints that return singleton/system data rather than listable collections.
...
func main() {
	explorer, err := files.InputManage.GetExplorer(
	...
	for _, object := range readObjects {
		...
		for _, field := range object.Fields {
			schemas.Add(providers.ModuleClioManage, objectName, displayName, object.URLPath, object.ResponseKey,
	...
	goutils.MustBeNil(files.OutputManage.SaveSchemas(schemas))
	goutils.MustBeNil(files.OutputManage.SaveQueryParamStats(scrapper.CalculateQueryParamStats(registry)))
