...
//
// Domain-scoped paths keep their placeholder ({domain_name}, or {authority_name}
// for domains/keys); the connector substitutes the workspace domain at request time.
package main
...
//
// gosec G101 flags the "credentials" path segment as a hardcoded credential;
// these are URL paths and object names, not secrets.
//
...
var getEndpoints = map[string]string{
	...
	// Alert settings records are the "events" array of the settings envelope;
	// they are created, updated and deleted at /v1/alerts/settings/events (see
	// write.go). One name covers both, per the same-object-across-operations rule.
...
// gosec G101: "domains/credentials" is an object name, not a secret.
//
//nolint:gochecknoglobals,gosec
var displayNameOverride = map[string]string{
	...
	"dynamic_pools/history":             "Dynamic IP Pool History",
	"thresholds/hits":                   "Threshold Hits",
...
// Objects extracts schemas for curated GET list endpoints and the POST analytics
// endpoints (logs, tags). ReadObjects is used (not ReadObjectsGet) so single-level {id} paths
