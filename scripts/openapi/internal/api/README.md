...
```text
        ...
        ┌────────────────────┼──────────────────┐
        ▼                    ▼                  ▼
   objectsMetadata.json   endpoints.json  queryParamsStats.json
        │
        ▼
 Connector.ListObjectMetadata
```
...
so that provider-specific scripts mostly describe **what they want**, rather than how to loop over and manipulate the data.
...
If no existing operation fits a genuinely provider-specific case, it is fine to work with the raw schemas and use a normal loop.

---

## Producing the standard output

The final pipeline is passed to the `output` package, which produces the standard artifacts used during connector development.

Every extraction script produces `objectsMetadata.json`. It contains the metadata for connector objects and their fields and satisfies the `ListObjectMetadata` connector method.
...
* **`objectsMetadata.json`** — object and field metadata used by `ListObjectMetadata`.
* **`endpoints.json`** — endpoint information used when implementing connector operations.
* **`queryParamsStats.json`** — consolidated query-parameter information.

These outputs form a developer knowledge base rather than runtime connector configuration.
