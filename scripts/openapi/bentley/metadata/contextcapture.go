...
var contextCaptureIgnoreEndpoints = []string{
	"/limits/engines",
}

func populateContextCapture(
	schemas *staticschema.Metadata[staticschema.FieldMetadataMapV2, any],
	registry datautils.NamedLists[string],
) {
	explorer, err := openapi.ContextCaptureFileManager.GetExplorer(
	...
	objects, err := explorer.ReadObjectsGet(
		api3.NewDenyPathStrategy(contextCaptureIgnoreEndpoints),
		nil, nil,
		api3.IdenticalObjectLocator,
	)
	goutils.MustBeNil(err)

	prefix := "contextcapture"
