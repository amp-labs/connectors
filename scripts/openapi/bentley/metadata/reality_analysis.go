...
var (
	realityAnalysisIgnoreEndpoints = []string{
		"/myprimaryaccount",
	}

	realityAnalysisObjectNameToResponseField = datautils.NewDefaultMap(map[string]string{
		"reality": "items",
	},
...
//nolint:dupl
func populateRealityAnalysis(
	schemas *staticschema.Metadata[staticschema.FieldMetadataMapV2, any],
	registry datautils.NamedLists[string],
) {
	explorer, err := openapi.RealityAnalysisFileManager.GetExplorer(
	...
	objects, err := explorer.ReadObjectsGet(
		api3.NewDenyPathStrategy(realityAnalysisIgnoreEndpoints),
		nil, nil,
		api3.CustomMappingObjectCheck(realityAnalysisObjectNameToResponseField),
	)
	goutils.MustBeNil(err)

	prefix := "reality-analysis"
