...
func main() {
	...
	for _, object := range Objects() {
		...
		for _, field := range object.Fields {
			...
			schemas.Add(providers.ModuleGoogleAds, objectName, object.DisplayName, urlPath,
	...
	goutils.MustBeNil(files.OutputAds.FlushSchemas(schemas))
	goutils.MustBeNil(files.OutputAds.SaveQueryParamStats(scrapper.CalculateQueryParamStats(registry)))

	slog.Info("Completed.")
}

func Objects() []metadatadef.Schema {
	explorer, err := files.InputAds.GetExplorer(
