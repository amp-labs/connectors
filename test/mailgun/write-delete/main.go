...
//   - routes: create -> update -> delete (form-encoded; read-shaped "actions"
//     list mapped to the repeated "action" field)
//   - forwards: create -> delete (query-parameter write; read-shaped nested
//     "forward" object flattened to forward.url / forward.recipient)
//   - templates: create -> update -> delete (form-encoded, domain-scoped)
//   - bounces: create -> delete (suppression: no update endpoint)
//   - alerts/settings: create -> update -> delete (JSON body, /events sub-path)
...
func testListsAndMembers(ctx context.Context, conn *mailgun.Connector) {
	...
	listAddress := "amp-live-test@" + testMailgun.GetWorkspace()
...
func testRoutes(ctx context.Context, conn *mailgun.Connector) {
	...
	// "actions" is the read-side key; the connector sends it as repeated "action".
	created := write(ctx, conn, common.WriteParams{
		ObjectName: "routes",
		RecordData: map[string]any{
...
func testForwards(ctx context.Context, conn *mailgun.Connector) {
	...
	// Nested "forward" is the read-side shape; the connector flattens it to
	// the forward.url query parameter.
	created := write(ctx, conn, common.WriteParams{
		ObjectName: "forwards",
		RecordData: map[string]any{
			"match": "amp-live-forward@" + testMailgun.GetWorkspace(),
			"forward": map[string]any{
				"urls": []any{"https://example.com/amp-live-test"},
			},
		},
	})
	if created == nil {
...
func testAlerts(ctx context.Context, conn *mailgun.Connector) {
	slog.Info("=== alerts/settings: create -> update -> delete (JSON write) ===")

	settings := map[string]any{"emails": []any{"integration.user+mailgun@withampersand.com"}}
	...
	defer remove(ctx, conn, "alerts/settings", created.RecordId)

	// PUT requires the full record (event_type, channel, settings).
	write(ctx, conn, common.WriteParams{
		ObjectName: "alerts/settings",
		RecordId:   created.RecordId,
		RecordData: map[string]any{
...
func testMessages(ctx context.Context, conn *mailgun.Connector) {
	...
	domain := testMailgun.GetWorkspace()
