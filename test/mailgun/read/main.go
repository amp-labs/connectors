...
func main() {
	...
	// is included because its timestamps are zone-less ISO 8601 (a different
	// format from the RFC 1123 used elsewhere).
	for _, obj := range []string{"domains", "keys"} {
		slog.Info("Testing incremental read", "object", obj)

		if err := testIncremental(ctx, conn, obj); err != nil {
			slog.Error(err.Error())
		}
	}
}
