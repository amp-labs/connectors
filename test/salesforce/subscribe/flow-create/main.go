...
//
// Watched fields are also the extra OM payload fields unless -watch-all is set.
// Watch Industry to capture it in the SOAP body:
//
//	go run ./test/salesforce/subscribe/flow-create -endpoint <url> -serve \
//	    -object Account:create,update:Industry
//
// -watch-all is watchFieldsAuto: all — no ISCHANGED formula. Pass -payload for
// WatchFieldsAutoFields (the read-selected fields the server will send):
//
//	go run ./test/salesforce/subscribe/flow-create -endpoint <url> -serve \
//	    -object Account -watch-all -payload Name,Industry
//
...
func main() {
	...
	flag.BoolVar(&watchAll, "watch-all", false,
		"WatchFieldsAll: no ISCHANGED formula; OM uses -payload (WatchFieldsAutoFields) instead of -watch")
	flag.StringVar(&payloadList, "payload", "",
		"OM payload fields when -watch-all is set, comma-separated (Id/CreatedDate/LastModifiedDate always included)")
	...
	if watchAll {
		payload := splitAndTrim(payloadList)
		for name, objEvents := range subscriptionEvents {
			objEvents.WatchFieldsAll = true
			objEvents.WatchFieldsAutoFields = payload
			subscriptionEvents[name] = objEvents
		}
	}
...
func printPlan(subscriptionEvents map[common.ObjectName]common.ObjectEvents, endpoint string) {
	...
	for _, name := range sortedObjectNames(subscriptionEvents) {
		...
		if objEvents.WatchFieldsAll {
			payload := "audit fields only"
			if len(objEvents.WatchFieldsAutoFields) > 0 {
				payload = "OM " + strings.Join(objEvents.WatchFieldsAutoFields, ", ")
			}

			watched = "watchFieldsAuto: all, " + payload
		} else if len(objEvents.WatchFields) > 0 {
...
//
// -update-watch changes both the flow entry condition and the outbound
// message field list (watch fields are included in the OM payload).
//
