...
func main() {
	...
	if err := testRead(ctx, conn, "crm-adnmg/currencies"); err != nil {
		slog.Error(err.Error())
	}

	if err := testRead(ctx, conn, "crm-evtmg/registrants"); err != nil {
		slog.Error(err.Error())
	}

	if err := testRead(ctx, conn, "crm-fndmg/fundraisingpurposes"); err != nil {
		slog.Error(err.Error())
	}
}
