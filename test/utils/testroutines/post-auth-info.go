...
func (r PostAuthInfo) Run(t *testing.T, builder ConnectorBuilder[connectors.AuthMetadataConnector]) {
	...
	r.Comparator = func(serverURL string, actual, expected *common.PostAuthInfo) *mockutils.CompareResult {
		...
		result.AddMismatch("ProviderWorkspaceRef", actual.ProviderWorkspaceRef, expected.ProviderWorkspaceRef)
		result.AddMismatch("CatalogVars", actual.CatalogVars, expected.CatalogVars)
		...
		if actual.RawResponse != nil && expected != nil {
			result.AddMismatch("RawResponse.Code", actual.RawResponse.Code, expected.RawResponse.Code)

			if expected.RawResponse.Headers != nil {
				result.AddDiff("RawResoponse.Headers is not supported by PostAuthInfo TestCase")
			}
		}
