...
func main() {
	...
	if err := readProducts(ctx, conn); err != nil {
		slog.Error(err.Error())
	}
}
...
func readProducts(ctx context.Context, conn *pipedrive.Connector) error {
	config := connectors.ReadParams{
		ObjectName: "products",
		Since:      time.Now().Add(-720 * time.Hour),
		Fields:     connectors.Fields("name", "code", "description", "id"),
	}
